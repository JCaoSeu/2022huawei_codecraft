...
char* GetIniKeyString(char* title, char* key, char* filename)
{
	...
	sprintf_s(sTitle, "[%s]", title);
	/*if (NULL == (fp = fopen_s(filename, "r"))) {
		perror("fopen");
		return NULL;
	}
*/
...
int main() {
	...
	//读取客户和节点间的qos关系
	ifstream infile3("..\\data\\site_bandwidth.csv", ios::in);
	if (!infile3.is_open())
	{
		cout << "读取文件失败3" << endl;
		return 0;
	}
	vector<vector<int>> qosMatrix(nodeNum);
	string line3;
