...
int main() {
	...
	vector<vector<int>> qosMatrix(nodeNum, vector<int>(clientNum));//qos矩阵
	string line3;
	string qos;
	getline(infile3, line3);//使用第一行来获取demand里客户和qos里客户的映射关系
	istringstream readstr3(line3);
	getline(readstr3, qos, ',');//去掉第一个字符串site_name
	vector<int> demandToQos(clientNum);//记录demand里客户和qos里客户的映射关系
	for (int i = 0; i < clientNum; ++i) {
		getline(readstr3, qos, ',');
		if (*qos.rbegin() == '\r') {
			qos = qos.substr(0, qos.size() - 1);
		}
		for (int j = 0; j < clientNum; ++j) {
			if (clientName[j] == qos) {
				demandToQos[i] = j;//第i列的数据是第j个客户的
				break;
			}
		}
	}
	...
	while (getline(infile3, line3)) {
		istringstream readstr3(line3);
		getline(readstr3, qos, ',');//使用第一个字符串来获取site_bandwidth里边缘节点和qos里边缘节点的映射关系
		int j;
		for (j = 0; j < nodeNum; ++j) {
			if (nodes[j].name == qos) {
				break;
		...
		for (int i = 0; i < clientNum; ++i) {
			getline(readstr3, qos, ',');
			qosMatrix[j][demandToQos[i]] = atoi(qos.c_str());
		}
	}
	...
	sort(priority.begin(), priority.end(), cmp3);
	...
	vector<vector<pair<int, int>>>used_time_assign(nodeNum, vector<pair<int, int>>(Time));//pair <t,assign>
	////轮流充满0.05Time的时刻
	...
	for (int i = 0; i < Time; ++i) {
		...
		for (int row = 0; row < nodeNum; ++row) {
			...
			AdjacencyMatrix[label][clientNum + nodeNum + 1] = (nodes[label - clientNum].bandwidth < limitFlow ? nodes[label - clientNum].bandwidth : limitFlow);//先把带宽限制为95百分位值
		}
		...
		*/
		...
		while ((increase = SPFA(cost_net, AdjacencyMatrix, pre, clientNum, nodeNum)) != 0) {
			maxflow += increase;
		...
		if (maxflow < total) {
			...
			for (int row = 0; row < nodeNum; ++row) {
				int label = priority[row].first + clientNum;
				AdjacencyMatrix[label][clientNum + nodeNum + 1] += (nodes[label - clientNum].bandwidth < limitFlow ? 0 : nodes[label - clientNum].bandwidth - limitFlow);//增加至原有带宽
				//AdjacencyMatrix[label][clientNum + nodeNum + 1] -= 5000;
			}
			...
			for (int n = 0; n < nodeNum; ++n) {
				int label = priority[n].first + clientNum;
				cost_net[label][clientNum + nodeNum + 1] = n;
				// int tmp=Time-(int)ceil((Time) * 0.95);
				// if (i+1<tmp) {
				// 	if(used[label][i]==0)//把该时刻标号为label的边缘节点的n95带宽存入priority数组用来排序
				// 		cost_net[label][clientNum + nodeNum + 1]*=2;
				// }
				// else {
				// 	if(used[label][i+1-tmp]==0)
				// 		cost_net[label][clientNum + nodeNum + 1]*=2;
		...
		for (int k = clientNum; k < nodeNum + clientNum; ++k) {
			int assign = AdjacencyMatrix[clientNum + nodeNum + 1][k];

			used[k - clientNum].push_back(assign);//每个边缘节点带宽使用为邻接矩阵中终点到边缘节点的值
			used_copy[k - clientNum].push_back(assign);
			pair<int, int>p = { i,assign };
			used_time_assign[k - clientNum][i] = p;
		}
		...
		for (auto& node : priority) {
			...
			int tmp = Time - (int)ceil((Time) * 0.95);
			if (i + 1 < tmp) {
				node.second = used[label][i];//把该时刻标号为label的边缘节点的n95带宽存入priority数组用来排序
			}
			else {
				node.second = used[label][i + 1 - tmp];
			}
	...
	//						int flow = record[t][client][label]/10;
	//						int turn = 0;
	//						while (turn <10 && node != label && qosMatrix[node][client] > 0 && used_copy[node][t] > priority[node].second && used_copy[node][t] + flow <= nodes[node].bandwidth) {
	...
	//	}
	//}
	/*for (auto& vec : used_time_assign) {
		sort(vec.begin(), vec.end(), cmp2);
	}
	int node = 0;
	for (auto& vec : used_time_assign) {

		int i = (int)ceil(Time * 0.95);
		if (vec[i - 1].second > vec[i - 2].second) {
			bool flag = false;
			int res = vec[i - 1].second - vec[i - 2].second;
			int now = vec[i - 1].first;
			for (int c = 0; c < clientNum; c++) {
				if (record[now][c][node] > 0) {
					for (int k = 0; k < nodeNum; k++) {
						if (k != node && qosMatrix[k][c] == 1) {
							if (used_copy[k][now] < used[k][i - 1]) {
								int res = (used[k][i - 1] - used_copy[k][now]);
								res = (res >= record[now][c][node]) ? record[now][c][node] : res;
								used_copy[k][now] += res;
								used[k] = used_copy[k];
								sort(used[k].begin(), used[k].end());
								vec[i - 1].second -= res;
								used_copy[node][now] -= res;
								used[node] = used_copy[node];
								sort(used[node].begin(), used[node].end());
								record[now][c][node] -= res;
								record[now][c][k] += res;
								flag = true;
								break;
							}
