...
int main() {
	...
	*/
	...
	vector<vector<int>> used_copy(nodeNum,vector<int>(Time));
	...
	for (int i = 0; i < Time; ++i) {
		...
		for (int row = 0; row < nodeNum; ++row) {
			...
			AdjacencyMatrix[label][clientNum + nodeNum + 1] = 4200;//先把带宽限制为95百分位值
		}
		...
		*/
		...
		if (maxflow < total) {
			...
			for (int row = 0; row < nodeNum; ++row) {
				int label = priority[row].first + clientNum;
				AdjacencyMatrix[label][clientNum + nodeNum + 1] += nodes[label-clientNum].bandwidth- 4200;//增加至原有带宽
			}
		...
		for (int k = clientNum; k < nodeNum + clientNum; ++k) {
			used[k - clientNum][0] = AdjacencyMatrix[clientNum + nodeNum + 1][k];//每个边缘节点带宽使用为邻接矩阵中终点到边缘节点的值
			used_copy[k - clientNum][i] = AdjacencyMatrix[clientNum + nodeNum + 1][k];
		}
		...
		for (auto& node : priority) {
			...
			node.second = used[label][n95];//把该时刻标号为label的边缘节点所用带宽存入priority数组用来排序
		}
				...
				for (int client = 0; client < clientNum; ++client) {
					if (record[t][client][label] > 0) {
						for (int node = 0; node < nodeNum; ++node) {
							int flow = record[t][client][label];
							if (node != label && qosMatrix[node][client] > 0 && used_copy[node][t] + flow < priority[node].second && used_copy[node][t] + flow <= nodes[node].bandwidth) {
